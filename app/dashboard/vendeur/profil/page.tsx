...
export default function SellerProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      setForm({
        ...
        matricule_fiscal: "", // not in UserProfile yet
        secteur: "",
        email: user.email ?? "",
        phone: user.phone ?? "",
        adresse_siege: "",
  ...
  const fields = [
    { label: "Raison sociale", key: "company_name" as const },
    { label: "Matricule fiscal", key: "matricule_fiscal" as const },
    { label: "Secteur", key: "secteur" as const },
    { label: "Email", key: "email" as const, readOnly: true },
    { label: "Téléphone", key: "phone" as const },
    { label: "Adresse", key: "adresse_siege" as const },
  ];
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <div className="rounded-xl border border-line bg-paper p-6">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-ink font-heading">Informations de l&apos;entreprise</h2>
            <button onClick={() => setEditMode((e) => !e)} className="text-sm font-semibold text-accent hover:text-accent-dark">
          ...
          {editMode && (
            <button onClick={handleSave} disabled={saving} className="mt-6 w-full rounded-md bg-accent py-2.5 text-sm font-semibold text-white transition-colors hover:bg-accent-dark disabled:opacity-60">
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-line bg-paper p-6">
            <h2 className="text-lg font-bold text-ink font-heading">KYC Entreprise</h2>
            <div className="mt-4 space-y-3">
              ...
              <div className="flex items-center justify-between rounded-lg bg-accent-tint px-4 py-3">
                <span className="text-sm font-medium text-accent">CIN du représentant</span>
              ...
              <div className="flex items-center justify-between rounded-lg bg-accent-tint px-4 py-3">
                <span className="text-sm font-medium text-accent">Patente</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-accent-tint px-4 py-3">
                <span className="text-sm font-medium text-accent">RIB entreprise</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-6">
            ...
            <button onClick={() => toast.error("Fonctionnalité à confirmer")} className="mt-4 w-full rounded-md bg-red-600 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-red-700">
    ...
    <div className="mt-4 space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-ink-muted">Envoyés sur le +216 71 123 456</p>
        </div>
